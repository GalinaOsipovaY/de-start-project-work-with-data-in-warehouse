...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. 
                             -- Этой информации в рамках расчётного периода раньше не было, это новые данные. 
    ...
    SELECT  
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины 
				           -- и применяем оконную функцию для определения самой популярной категории товаров
                        *,
                        ...
                        FROM (                        							
							SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, 
							       -- кроме столбца с самой популярной категорией товаров  
								...
								INNER JOIN (select -- Самый популярный мастер определяется по количеству заказов от заказчика
								                   a_customer_id as b_customer_id, 
								                   b.craftsman_id as craftsman_id
								            from (select dd_customer_id as a_customer_id, 
								                         a.craftsman_id, 
								                         dd_count_order as a_count_order,
											             row_number () over (partition by dd_customer_id order by dd_count_order desc) as rn
												  from (
												        select distinct dd.customer_id as dd_customer_id, 
												                        dd.craftsman_id,
														                count (dd.order_id) over (partition by dd.customer_id, dd.craftsman_id) as dd_count_order
														from dwh_delta as dd
														) as a 
												  ) as b where rn = 1
									       ) AS T5 ON T2.customer_id = T5.b_customer_id
...
dwh_delta_update_result AS ( -- делаем перерасчёт для существующих записей витринs, так как данные обновились за отчётные периоды. 
